...
use std::io;
use std::io::{Seek, Read, BufReader, Write, BufWriter, SeekFrom};
use std::ops::Range;
...
impl KvStore {
    pub fn open(path: impl Into<PathBuf>) -> Result<KvStore> {
        ...
        // return sorted generation number in given directory
        ...
        fn load(
            ...
            // make sure read from file beginning
            let mut pos = reader.seek(SeekFrom::Current(0))?;
            let mut stream = Deserializer::from_reader(reader).into_iter::<Command>();
            let mut uncompacted = 0;
            while let Some(cmd) = stream.next() {
                let new_pos = stream.byte_offset() as u64;
                match cmd? { // if cmd is None, "?" will return None
                    Command::Set{key, ..} => {
        ...
        for &gen in &gen_list {
            let mut reader = BufferReaderWithPos::new(
                File::open(
                    log_path(&path, gen)
                )?
            )?;
