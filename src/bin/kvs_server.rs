...
use rustdb::engines;
use rustdb::engines::{KvsEngine, KvStore, SledKvsEngine};
use rustdb::KvsError::Sled;
use rustdb::Result;
use rustdb::server::KvsServer;
...
fn main() {
    ...
    let res = current_engine().and_then(move |current_engine()| {
        if opt.engine.is_none() {
            opt.engine = current_engine;
        }
        if current_engine.is_some() && opt.engine != current_engine {
            error!("wrong engine");
            exit(1);
        }
        run(opt)
